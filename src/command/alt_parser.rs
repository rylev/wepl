...
impl<'a> FunctionCall<'a> {
    fn try_parse(input: &mut VecDeque<Token<'a>>) -> Result<Option<Self>, ParserError<'a>> {
        ...
        let Some(function_ident) = Ident::try_parse(input)? else {
            return Ok(None);
        };
...
// TODO: Rename this to Ident and Ident to ItemIdent
...
impl<'a> ItemIdent<'a> {
    pub(crate) fn try_parse(
        input: &mut VecDeque<Token<'a>>,
    ) -> Result<Option<ItemIdent<'a>>, ParserError<'a>> {
        match Ident::try_parse(input)? {
